<?php 
	...
	class FrontendModel extends ConnectDB{
		...
		function rate($productId, $userId, $star) {
			$sql = "INSERT INTO rates(product_id, user_id, star) VALUES($productId, $userId, $star)";
			//var_dump($sql);exit();
			return mysqli_query($this->connect(), $sql);
		}
		function avgstar($product_id)
		{
			$sql = "SELECT AVG(star) FROM rates WHERE id";
			return mysqli_query($this->connect(), $sql);
		}
	}
?>